...
Route::get('/begin', function (\Illuminate\Http\Request $request) {
    ...
    return redirect()->route('authorize-integration', $request->get('token'));
...
Route::get('/authorize/{integrationToken}', function (\Illuminate\Http\Request $request, $integrationToken) {
    ...
    $client->setupPluginSettingsPage($appSlug, route('settings-primecargo', [$connection->rackbeat_user_account_id, $connection->internal_token]));

    // consider: do we need webhooks?
    $client->setupWebhook('order.shipped', route('webhooks.order-shipped', [$connection->rackbeat_user_account_id, $connection->internal_token]));
